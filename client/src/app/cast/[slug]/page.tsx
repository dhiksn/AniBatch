...
export default function CastPage() {
  ...
  function load(p: number) {
    ...
    setLoading(true);
    setError(null);
    window.history.replaceState(null, '', p > 1 ? `/cast/${slug}?page=${p}` : `/cast/${slug}`);
    fetchApi<any>(`/cast/${slug}?page=${p}`)
      .then((res) => {
        setData(res.data ?? []);
      ...
      .catch((err) => {
        setError(err.message || "Gagal memuat data cast");
        setLoading(false);
      });
  }
  ...
  return (
    <main className="w-full">
      <div className="flex items-center gap-2 mb-8 bg-stone-900/40 p-6 rounded-2xl border border-stone-800/50">
        <Users weight="fill" className="text-brand-500 text-3xl" />
        <div>
          <h1 className="text-2xl font-black text-stone-100 tracking-tight">
            Cast: <span className="text-brand-500">{castInfo?.name || slug}</span>
          </h1>
          <p className="text-sm text-stone-400 mt-1">Daftar anime dengan cast {castInfo?.name || slug}</p>
        </div>
      </div>
      ...
      {loading ? <CardGridSkeleton count={20} /> : error ? (
        ...
        <div className="text-center py-20 text-stone-500">Tidak ada anime ditemukan untuk cast ini.</div>
      )}
    </main>
  );
}
