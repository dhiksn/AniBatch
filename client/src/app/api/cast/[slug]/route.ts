import { getAnimeByCast } from '@/lib/scraper/index.js';
...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!slug || !/^[\w-]+$/.test(slug))
    return errorResponse(400, 'BAD_REQUEST', 'Slug cast tidak valid');
  const { searchParams } = new URL(request.url);
  const page = Math.max(1, parseInt(searchParams.get('page') || '1', 10) || 1);
  try {
    const { name, animes, pagination } = await getAnimeByCast(slug, page) as any;
